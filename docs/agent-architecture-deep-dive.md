...
## 9. 补充：移动端主链 + Web Skill 旁路

### 职责
- 在不破坏移动端执行主链的前提下，增加 Web 子任务执行能力。

### 上游输入
- `intent_key` 任务域分类（mobile/web）
- 当前会话上下文（是否处于 App 内 H5 或外部网页流程）

### 核心机制
- 默认走 `mobile_native`（ADB 主链）。
- 仅 `web` 子任务通过 `AgentBrowserSkill` 走 `web_skill`。
- `web_skill` 失败后回退到主链并触发 `handover`/重规划。

### 下游影响
- 增强网页流程执行成功率，同时维持移动端系统动作稳定性。

### 优点
- 复用社区成熟 Web 执行能力且不破坏现有链路。

### 缺点/风险
- 新增路由层与外部进程集成复杂度，需严格事件审计。
