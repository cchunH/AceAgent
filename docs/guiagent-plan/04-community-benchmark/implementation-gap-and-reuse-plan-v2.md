...
- 现状：支持 watchdog 插件启停、最小严重级过滤、去重窗口、节流窗口、`escalation_rules`、`cross_task_aggregation` 与热重载参数。
- 距离评估：`完成（v1）`
...
4. 已完成：watchdog 跨任务聚合（`cross_task_aggregation`）。
5. 待继续：事件 schema 版本迁移策略（`v1 -> v2`）与兼容校验测试。
6. 待继续：watchdog 聚合统计导出与可视化指标。
